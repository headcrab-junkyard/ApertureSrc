...
{
	//virtual ~IRenderModelManager() = default; // TODO: bad idea
	...
	virtual void FreeModel(idRenderModel *apModel) = 0;

	// returns NULL if asName is NULL or an empty string, otherwise
	/// it will create a default model if not loadable
	virtual idRenderModel *FindModel(const char *asName) = 0;

	/// returns NULL if not loadable
	virtual idRenderModel *CheckModel(const char *asName) = 0;
	...
	virtual void AddModel(idRenderModel *apModel) = 0;
	...
	virtual void RemoveModel(idRenderModel *apModel) = 0;

	/// the reloadModels console command calls this, but it can
	/// also be explicitly invoked
	virtual void ReloadModels(bool abForceAll = false) = 0;

	/// write "touchModel <model>" commands for each non-world-map model
	virtual void WritePrecacheCommands(IFile *apFIle) = 0;
...
extern IRenderModelManager *renderModelManager;