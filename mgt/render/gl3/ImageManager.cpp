...
CImageManager *globalImages = &CImageManager;

CConVar preLoad_Images("preLoad_Images", "1", CVAR_SYSTEM | CVAR_BOOL, "preload images during beginlevelload");
...
==============
*/
idImage *CImageManager::AllocImage(const char *name)
{
	if(strlen(name) >= MAX_IMAGE_NAME)
		mpSystem->Error("CImageManager::AllocImage: \"%s\" is too long\n", name);

	int hash{idStr(name).FileNameHash()};

	idImage *pImage{new(TAG_IMAGE) idImage(name)};

	imageHash.Add(hash, images.Append(pImage));
...
==============
*/
idImage *CImageManager::AllocStandaloneImage(const char *name)
{
	if(strlen(name) >= MAX_IMAGE_NAME)
		mpSystem->Error("CImageManager::AllocImage: \"%s\" is too long\n", name);
...
==================
*/
idImage *CImageManager::ImageFromFunction(const char *_name, void (*generatorFunction)(idImage *image))
{
...
==============
*/
idImage *CImageManager::GetImageWithParameters(const char *_name, textureFilter_t filter, textureRepeat_t repeat, textureUsage_t usage, cubeFiles_t cubeMap) const
{
...
==============
*/
idImage *CImageManager::LoadFromFile(const char *_name, textureFilter_t filter, textureRepeat_t repeat, textureUsage_t usage, cubeFiles_t cubeMap)
{
	if(!_name || !_name[0] || idStr::Icmp(_name, "default") == 0 || idStr::Icmp(_name, "_default") == 0)
	{
		declManager->MediaPrint("DEFAULTED\n");
...
/*
========================
CImageManager::ScratchImage
========================
*/
idImage *CImageManager::ScratchImage(const char *_name, idImageOpts *imgOpts, textureFilter_t filter, textureRepeat_t repeat, textureUsage_t usage)
{
	if(!_name || !_name[0])
		mpSystem->FatalError("CImageManager::ScratchImage called with empty name");

	if(imgOpts == nullptr)
		mpSystem->FatalError("CImageManager::ScratchImage called with NULL imgOpts");
...
/*
===============
CImageManager::GetImage
===============
*/
idImage *CImageManager::GetImage(const char *_name) const
{
	if(!_name || !_name[0] || idStr::Icmp(_name, "default") == 0 || idStr::Icmp(_name, "_default") == 0)
	{
		declManager->MediaPrint("DEFAULTED\n");
...
===============
*/
void CImageManager::PurgeAllImages()
{
	idImage *image{nullptr};
...
===============
*/
void CImageManager::ReloadImages(bool all)
{
...
void R_CombineCubeImages_f(const idCmdArgs &args)
{
	if(args.Argc() != 2)
	{
		gpSystem->Printf("usage: combineCubeImages <baseName>\n");
		gpSystem->Printf(" combines basename[1-6][0001-9999].tga to basenameCM[0001-9999].tga\n");
		gpSystem->Printf(" 1: forward 2:right 3:back 4:left 5:up 6:down\n");
		return;
	};
...
void CImageManager::Init()
{
	...
	mpCmdRegistry->Add("reloadImages", R_ReloadImages_f, CMD_FL_RENDERER, "reloads images");
	mpCmdRegistry->Add("listImages", R_ListImages_f, CMD_FL_RENDERER, "lists images");
	mpCmdRegistry->Add("combineCubeImages", R_CombineCubeImages_f, CMD_FL_RENDERER, "combines six images for roq compression");
...
/*
====================
CImageManager::BeginLevelLoad
Frees all images used by the previous level
====================
*/
void CImageManager::BeginLevelLoad()
{
...
/*
====================
CImageManager::ExcludePreloadImage
====================
*/
bool CImageManager::ExcludePreloadImage(const char *name)
{
...
/*
====================
CImageManager::Preload
====================
*/
void CImageManager::Preload(const idPreloadManifest &manifest, const bool &mapPreload)
{
	if(preLoad_Images.GetBool() && manifest.NumResources() > 0)
	{
		// preload this levels images
		mpSystem->Printf("Preloading images...\n");
		...
		//mpFileSystem->StopPreload();
		int end = Sys_Milliseconds();
		
		mpSystem->Printf("%05d images preloaded ( or were already loaded ) in %5.1f seconds\n", numLoaded, (end - start) * 0.001);
		mpSystem->Printf("----------------------------------------\n");
...
/*
===============
CImageManager::LoadLevelImages
===============
*/
int CImageManager::LoadLevelImages(bool abPacifier)
{
	int loadCount{0};
	for(int i = 0; i < images.Num(); ++i)
	{
...
/*
===============
CImageManager::EndLevelLoad
===============
*/
void CImageManager::EndLevelLoad()
{
	insideLevelLoad = false;

	mpSystem->Printf("----- CImageManager::EndLevelLoad -----\n");
	int start = Sys_Milliseconds();
	int loadCount = LoadLevelImages(true);

	int end = Sys_Milliseconds();
	mpSystem->Printf("%5i images loaded in %5.1f seconds\n", loadCount, (end - start) * 0.001);
	mpSystem->Printf("----------------------------------------\n");
...
/*
===============
CImageManager::StartBuild
===============
*/
void CImageManager::StartBuild()
{
};

/*
===============
CImageManager::FinishBuild
===============
*/
void CImageManager::FinishBuild(bool removeDups)
{
};

/*
===============
CImageManager::PrintMemInfo
===============
*/
void CImageManager::PrintMemInfo(MemInfo_t *mi)
{
	int i, j, total = 0;
	int *sortIndex;
	IFile *f;

	f = mpFileSystem->OpenFileWrite(mi->filebase + "_images.txt");
	if(!f)
		return;
