...
public:
	idMD5Joint() = default;
	idStr name;
	const idMD5Joint *parent{nullptr};
};
...
{
	//virtual ~idRenderModel(){}; // NOPE

	// Loads static models only, dynamic models must be loaded by the modelManager
	virtual void InitFromFile(const char *fileName) = 0;

	// Supports reading/writing binary file formats
	virtual bool LoadBinaryModel(IFile *file, const ID_TIME_T sourceTimeStamp) = 0;
	virtual void WriteBinaryModel(IFile *file, ID_TIME_T *_timeStamp = NULL) const = 0;
