...
void CMonsterBrute::ShamClaw(float side)
{
	vec3_t delta;
	float ldmg;
	...
	self->EmitSound(CHAN_VOICE, "shambler/smack.wav", 1, ATTN_NORM);
...
void CMonsterBrute::Melee()
{
	float chance;

	chance = random();
	
	if(chance > 0.6 || self->GetHealth() == 600)
		sham_smash1();
	else if(chance > 0.3)
		sham_swingr1();
	else
...
void CMonsterBrute::CastLightning()
{
	vec3_t org, dir;
	...
	org = self->GetOrigin() + '0 0 40';

	dir = self->GetEnemy()->GetOrigin() + '0 0 16' - org;
	dir = normalize(dir);

	traceline(org, self->GetOrigin() + dir * 600, TRUE, self);
...
void() sham_magic10 = [$magic10, sham_magic11] { CastLightning(); };
...
void CMonsterBrute::Pain(CBaseEntity *attacker, float damage)
{
	self->EmitSound(CHAN_VOICE, "monsters/brute/shurt2.wav", 1, ATTN_NORM);
...
void CMonsterBrute::Die()
{
	...
	if(self->GetHealth() < -60)
	{
		...
		ThrowHead("models/h_shams.mdl", self->GetHealth());
	...
	// regular death
	self->EmitSound(CHAN_VOICE, "monsters/brute/sdeath.wav", 1, ATTN_NORM);
...
*/
C_EXPORT void monster_shambler(entvars_t *self)
{
	if(deathmatch)
	{
		remove(self);
	...
	precache_model("models/monsters/brute.mdl");
	precache_model("models/s_light.mdl");
	precache_model("models/h_brute.mdl");
	precache_model("models/bolt.mdl");

	precache_sound("monsters/brute/sattck1.wav");
	precache_sound("monsters/brute/sboom.wav");
	precache_sound("monsters/brute/sdeath.wav");
	precache_sound("monsters/brute/shurt2.wav");
	precache_sound("monsters/brute/sidle.wav");
	precache_sound("monsters/brute/ssight.wav");
	precache_sound("monsters/brute/melee1.wav");
	precache_sound("monsters/brute/melee2.wav");
	precache_sound("monsters/brute/smack.wav");

	self->solid = SOLID_SLIDEBOX;
	self->SetMoveType(MOVETYPE_STEP);
	
	self->SetModel("models/monsters/brute.mdl");
