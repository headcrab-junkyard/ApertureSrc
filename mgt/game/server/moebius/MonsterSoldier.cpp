...
void army_pain(CBaseEntity *attacker, float damage)
{
	float r;
	...
	if(r < 0.2)
	{
		...
		army_pain1();
		self->EmitSound(CHAN_VOICE, "soldier/pain1.wav", 1, ATTN_NORM);
	}
	else if(r < 0.6)
	{
		self.pain_finished = time + 1.1;
		army_painb1();
		self->EmitSound(CHAN_VOICE, "soldier/pain2.wav", 1, ATTN_NORM);
	}
	else
	{
		self.pain_finished = time + 1.1;
...
void() army_die3 = [$death3, army_die4] {self.solid = SOLID_NOT;self.ammo_shells = 5;DropBackpack(); };
...
void() army_cdie3 = [$deathc3, army_cdie4] {self.solid = SOLID_NOT;self.ammo_shells = 5;DropBackpack();ai_back(4); };
