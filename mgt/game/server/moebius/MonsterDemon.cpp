...
$cd / raid / quake / id1 / models / demon3
                                    $scale 0.8 $origin 0 0 24 $base base
                                    $skin base
...
void CMonsterDemon::MeleeAttack()
{
	demon1_atta1();
};
...
*/
C_EXPORT void monster_demon1(entvars_t *self)
{
	CMonsterDemon::Spawn();
};

void CMonsterDemon::Spawn()
{
	if(deathmatch)
	{
		remove(self);
	...
	setmodel(self, "models/monsters/demon.mdl");
...
bool DemonCheckAttack()
{
	...
	if(CheckDemonJump())
	{
		...
		self->EmitSound(CHAN_VOICE, "demon/djump.wav", 1, ATTN_NORM);
		return true;
	};
...
void Demon_JumpTouch()
{
	...
	self->SetThinkCallback(demon1_jump11);
