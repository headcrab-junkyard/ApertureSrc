...
class COgreGrenade : public CBaseEntity
{
public:
...
void COgreGrenade::Explode()
{
	T_RadiusDamage(self, self->GetOwner(), 40, world);
	
	self->EmitSound(CHAN_VOICE, "weapons/r_exp3.wav", 1, ATTN_NORM);

	WriteByte(MSG_BROADCAST, SVC_TEMPENTITY);
	WriteByte(MSG_BROADCAST, TE_EXPLOSION);
	WriteCoord(MSG_BROADCAST, self->origin[0]);
	WriteCoord(MSG_BROADCAST, self->origin[1]);
	WriteCoord(MSG_BROADCAST, self->origin[2]);

	self->SetVelocity('0 0 0');
	
	self->SetTouchCallback(SUB_Null);
	
	self->SetModel("progs/s_explod.spr");
	
	self->solid = SOLID_NOT;
	s_explode1();
};

void COgreGrenade::Touch(CBaseEntity *other)
{
	...
	self->EmitSound(CHAN_VOICE, "weapons/bounce.wav", 1, ATTN_NORM); // bounce sound
...
void OgreFireGrenade()
{
	...
	self->EmitSound(CHAN_WEAPON, "weapons/grenade.wav", 1, ATTN_NORM);

	CBaseEntity *missile = spawn();
	...
	missile.velocity = normalize(self->GetEnemy()->GetOrigin() - self->GetOrigin());
	missile.velocity = missile->GetVelocity() * 600;
	missile.velocity[2] = 200;

	missile.avelocity = '300 300 300';

	missile->SetAngles(vectoangles(missile.GetVelocity()));

	missile->SetTouchCallback(COgreGrenade::Touch);

	// set missile duration
	missile->SetNextThink(time + 2.5);
	missile->SetThinkCallback(COgreGrenade::Explode);

	missile->SetModel("models/grenade.mdl");
	missile->SetSize('0 0 0', '0 0 0');
	missile->SetOrigin(self->GetOrigin());
};

//=============================================================================

class CMonsterRipper : public CBaseMonster
{
public:
	void Spawn() override;
	
	void Pain(CBaseEntity *attacker, float damage);
	void Die();
private:
	void Melee();
...
void CMonsterRipper::Spawn()
{
	...
	precache_model("models/monsters/ripper.mdl");
	precache_model("models/h_ripper.mdl"); // head
	precache_model("models/grenade.mdl");

	precache_sound("monsters/ripper/ogdrag.wav");
	precache_sound("monsters/ripper/ogdth.wav");
	precache_sound("monsters/ripper/ogidle.wav");
	precache_sound("monsters/ripper/ogidle2.wav");
	precache_sound("monsters/ripper/ogpain1.wav");
	precache_sound("monsters/ripper/ogsawatk.wav");
	precache_sound("monsters/ripper/ogwake.wav");

	self->solid = SOLID_SLIDEBOX;
	self->SetMoveType(MOVETYPE_STEP);

	self->SetModel("models/monsters/ripper.mdl");

	self->SetSize(VEC_HULL2_MIN, VEC_HULL2_MAX);
	...
	self->th_die = CMonsterRipper::Die;
	self->th_melee = CMonsterRipper::Melee;
	self->th_missile = ogre_nail1;
	self->th_pain = CMonsterRipper::Pain;
...
================
*/
void CMonsterRipper::Chainsaw(float side)
{
	vec3_t delta;
	float ldmg;
	...
	if(!CanDamage(self->GetEnemy(), self))
		return;

	ai_charge(self, 10);

	delta = self->GetEnemy()->GetOrigin() - self->GetOrigin();

	if(vlen(delta) > 100)
		return;

	ldmg = (random() + random() + random()) * 4;
	T_Damage(self->GetEnemy(), self, self, ldmg);

	if(side)
	{
		makevectors(self->angles);
		if(side == 1)
			SpawnMeatSpray(self->GetOrigin() + v_forward * 16, crandom() * 100 * v_right);
		else
			SpawnMeatSpray(self->GetOrigin() + v_forward * 16, side * v_right);
	};
};
...
void CMonsterRipper::Pain(CBaseEntity *attacker, float damage)
{
	float r;
	...
	self->EmitSound(CHAN_VOICE, "ogre/ogpain1.wav", 1, ATTN_NORM);
...
void() ogre_die3 = [$death3, ogre_die4] {self->solid = SOLID_NOT;
...
void() ogre_bdie3 = [$bdeath3, ogre_bdie4] {self->solid = SOLID_NOT; self->ammo_rockets = 2;DropBackpack(); };
...
void CMonsterRipper::Die()
{
	// check for gib
	if(self->GetHealth() < -80)
	{
		self->EmitSound(CHAN_VOICE, "player/udeath.wav", 1, ATTN_NORM);
	
		ThrowHead("models/h_ogre.mdl", self->GetHealth());
	...
	self->EmitSound(CHAN_VOICE, "monsters/ripper/ogdth.wav", 1, ATTN_NORM);
...
/*QUAKED monster_ripper (1 0 0) (-32 -32 -24) (32 32 64) Ambush

*/
C_EXPORT void monster_ripper(entvars_t *self)
{
	CMonsterRipper::Spawn();
};

C_EXPORT void monster_ripper_marksman(entvars_t *self)
{
	monster_ripper(self);
};