...
void GotoNextMap()
{
	...
	//gpEngine->pfnChangeLevel (mapname);
	//else
	{
		...
		if(newmap != "")
			gpEngine->pfnChangeLevel(newmap);
		else
			gpEngine->pfnChangeLevel(nextmap);
	}
};
...
void CChangeLevel::Touch(CBaseEntity *other)
{
	entity pos;
	...
	self->SetThinkCallback(CChangeLevel::Execute);
	self->SetNextThink(time + 0.1);
};
...
C_EXPORT void trigger_changelevel(entvars_t *self)
{
	...
	InitTrigger();
	self->SetTouchCallback(CChangeLevel::Touch);
};
...
{
	CBaseEntity *spot, newspot, thing;
...
void NextLevel()
{
	...
	if(o->GetNextThink() < time)
	{
		o->SetThinkCallback(CChangeLevel::Execute);
		o->SetNextThink(time + 0.1);
	};
};
...
================
*/
void PlayerPreThink(int clientid)
{
	float mspeed, aspeed;
	float r;
	...
	if(gPlayers[clientid]->v.view_ofs == '0 0 0')
		return; // intermission or finale

	makevectors(gPlayers[clientid]->v.v_angle); // is this still used

	gPlayers[clientid]->v.deathtype = "";

	CheckRules(self);
	gPlayers[clientid]->WaterMove();
	
	//if(gPlayers[clientid]->GetWaterLevel() == 2)
		//gPlayers[clientid]->CheckWaterJump();

	if(gPlayers[clientid]->v.deadflag >= DEAD_DEAD)
	{
		gPlayers[clientid]->DeathThink();
		return;
	};

	if(gPlayers[clientid]->v.deadflag == DEAD_DYING)
		return; // dying, so do nothing

	if(gPlayers[clientid]->v.button2)
		gPlayers[clientid]->PlayerJump();
	else
		gPlayers[clientid]->v.flags |= FL_JUMPRELEASED;

	// teleporters can force a non-moving pause time
	if(time < gPlayers[clientid]->v.pausetime)
		gPlayers[clientid]->v.velocity = '0 0 0';

	if(time > gPlayers[clientid]->v.attack_finished && gPlayers[clientid]->v.currentammo == 0 && gPlayers[clientid]->v.weapon != IT_AXE)
	{
		gPlayers[clientid]->v.weapon = W_BestWeapon();
		W_SetCurrentAmmo();
	};
};
