...
void play_teleport()
{
	...
	gpEngine->pfnEmitSound (self, CHAN_VOICE, tmpstr, 1, ATTN_NORM);
	remove (self);
};
...
void spawn_tdeath(vector org, entity death_owner)
{
	...
	gpEngine->pfnSetSize (death, death_owner.mins - '1 1 1', death_owner.maxs + '1 1 1');
	gpEngine->pfnSetOrigin (death, org);
...
void trigger_teleport()
{
	...
	if (!(self.spawnflags & SILENT))
	{
		...
		gpEngine->pfnEmitAmbientSound (o, "ambience/hum1.wav",0.5 , ATTN_STATIC);
	}
};
	...
	else if (other.health > 0)
	{
		...
		if (other.classname == "player")
		{
			if (other.fly_sound < time)
			{
				...
				gpEngine->pfnEmitSound (other, CHAN_AUTO, "ambience/windfly.wav", 1, ATTN_NORM);
			}
		}
	}
