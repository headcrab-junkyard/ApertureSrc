...
void M_Menu_Setup_f()
{
	...
	setup_top = setup_oldtop = ((int)cl_color.GetValue()) >> 4;
	setup_bottom = setup_oldbottom = ((int)cl_color.GetValue()) & 15;
}
...
void M_AdjustSliders(int dir)
{
	...
	{
	case 3: // screen size
		scr_viewsize.SetValue(scr_viewsize.GetValue() + dir * 10);
		if(scr_viewsize.GetValue() < 30)
			scr_viewsize.SetValue(30);
		if(scr_viewsize.GetValue() > 120)
			scr_viewsize.SetValue(120);
		Cvar_SetValue("viewsize", scr_viewsize.GetValue());
		break;
	case 4: // gamma
		v_gamma.SetValue(v_gamma.GetValue() - dir * 0.05);
		if(v_gamma.GetValue() < 0.5)
			v_gamma.SetValue(0.5);
		if(v_gamma.GetValue() > 1)
			v_gamma.SetValue(1);
		Cvar_SetValue("gamma", v_gamma.GetValue());
		break;
	case 5: // mouse speed
		sensitivity.SetValue(sensitivity.GetValue() + dir * 0.5);
		if(sensitivity.GetValue() < 1)
			sensitivity.SetValue(1);
		if(sensitivity.GetValue() > 11)
			sensitivity.SetValue(11);
		Cvar_SetValue("sensitivity", sensitivity.GetValue());
		break;
	case 6: // music volume
#ifdef _WIN32
		bgmvolume.SetValue(bgmvolume.GetValue() + dir * 1.0);
#else
		bgmvolume.SetValue(bgmvolume.GetValue() + dir * 0.1);
#endif
		if(bgmvolume.GetValue() < 0)
			bgmvolume.SetValue(0);
		if(bgmvolume.GetValue() > 1)
			bgmvolume.SetValue(1);
		Cvar_SetValue("bgmvolume", bgmvolume.GetValue()); // TODO: oh...
		break;
	case 7: // sfx volume
		volume.GetValue() += dir * 0.1;
		if(volume.GetValue() < 0)
			volume.GetValue() = 0;
		if(volume.GetValue() > 1)
			volume.GetValue() = 1;
		Cvar_SetValue("volume", volume.GetValue());
		break;

	case 8: // allways run
		if(cl_forwardspeed.GetValue() > 200)
		{
	...
	case 9: // invert mouse
		Cvar_SetValue("m_pitch", -m_pitch.GetValue());
		break;

	case 10: // lookspring
		Cvar_SetValue("lookspring", !lookspring.GetValue());
		break;

	case 11: // lookstrafe
		Cvar_SetValue("lookstrafe", !lookstrafe.GetValue());
		break;

#ifdef _WIN32
	case 13: // _windowed_mouse
		Cvar_SetValue("_windowed_mouse", !_windowed_mouse.GetValue());
		break;
#endif
	}
}
...
void M_Options_Draw()
{
	...
	r = (scr_viewsize.GetValue() - 30) / (120 - 30);
	M_DrawSlider(220, 56, r);

	M_Print(16, 64, "            Brightness");
	r = (1.0 - v_gamma.GetValue()) / 0.5;
	M_DrawSlider(220, 64, r);

	M_Print(16, 72, "           Mouse Speed");
	r = (sensitivity.GetValue() - 1) / 10;
	M_DrawSlider(220, 72, r);

	M_Print(16, 80, "       CD Music Volume");
	r = bgmvolume.GetValue();
	...
	M_DrawCheckbox(220, 96, cl_forwardspeed.GetValue() > 200);

	M_Print(16, 104, "          Invert Mouse");
	M_DrawCheckbox(220, 104, m_pitch.GetValue() < 0);

	M_Print(16, 112, "            Lookspring");
	M_DrawCheckbox(220, 112, lookspring.GetValue());

	M_Print(16, 120, "            Lookstrafe");
	M_DrawCheckbox(220, 120, lookstrafe.GetValue());
	...
	if(modestate == MS_WINDOWED)
	{
		...
		M_DrawCheckbox(220, 136, _windowed_mouse.GetValue());
	}
#endif
...
void M_GameOptions_Draw()
{
	...
	*/
	...
	else if(skill.GetValue() == 1)
		M_Print(160, 80, "Normal difficulty");
	else if(skill.GetValue() == 2)
		M_Print(160, 80, "Hard difficulty");
	else
		M_Print(160, 80, "Nightmare difficulty");

	M_Print(0, 88, "       Frag Limit");
	if(fraglimit.GetValue() == 0)
		M_Print(160, 88, "none");
	else
		M_Print(160, 88, va("%i frags", (int)fraglimit.GetValue()));

	M_Print(0, 96, "       Time Limit");
	if(timelimit.GetValue() == 0)
		M_Print(160, 96, "none");
	else
		M_Print(160, 96, va("%i minutes", (int)timelimit.GetValue()));
...
void M_NetStart_Change(int dir)
{
	...
	case 2:
		Cvar_SetValue("coop", coop.GetValue() ? 0 : 1);
		break;

	case 3:
		count = 2;

		Cvar_SetValue("teamplay", teamplay.GetValue() + dir);
		if(teamplay.GetValue() > count)
			Cvar_SetValue("teamplay", 0);
		else if(teamplay.GetValue() < 0)
			Cvar_SetValue("teamplay", count);
		break;

	case 4:
		Cvar_SetValue("skill", skill.GetValue() + dir);
		if(skill.GetValue() > 3)
	...
	case 5:
		Cvar_SetValue("fraglimit", fraglimit.GetValue() + dir * 10);
		if(fraglimit.GetValue() > 100)
	...
	case 6:
		Cvar_SetValue("timelimit", timelimit.GetValue() + dir * 5);
		if(timelimit.GetValue() > 60)
